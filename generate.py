...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for v in self.crossword.variables:
            domain_of_v = self.domains[v]
            for value in domain_of_v.copy():
                if len(value) != v.length:
                    domain_of_v.remove(value)
    ...
    def revise(self, x, y):
        ...
        if overlaps[(x, y)] != None:
            ...
            for x_domain_val in self.domains[x].copy():
                for y_domain_val in self.domains[y].copy():
                    if x_domain_val[x_matching_letter] == y_domain_val[y_matching_letter]:
                        matches.append(x_domain_val)
            # remove non-matches from domain
    ...
    def ac3(self, arcs=None):
        ...
        # If we have removed something from x in revise(x, y), some value in 
